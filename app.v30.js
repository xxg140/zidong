...
const DB = {
  ...
  set(key, v) {
    try {
      ...
      if (saved !== JSON.stringify(v)) {
        console.warn('DB.set verification failed for', key);
      }
    } catch(e) {
      this._lastError = e;
      console.error('DB.set error for', key, e);
      alert('存储失败：' + e.message + '\n请检查浏览器存储空间或关闭隐私/无痕模式');
    }
  },
  add(key, item) {
    try {
...
function init() {
  ...
  loadData();
  console.log('[init] 加载数据完成，tasks数量:', state.tasks.length, 'contacts数量:', state.contacts.length);
  loadSampleData();
  setupEventListeners();
  renderGroupOptions();
  renderAll();
  console.log('[init] 初始化完成，tasks数量:', state.tasks.length);
}
...
function setupEventListeners() {
  ...
  document.querySelectorAll('.detail-tab').forEach(btn => btn.addEventListener('click', () => switchDetailTab(btn.dataset.detailTab)));
...
function confirmManualAdd() {
  ...
  const added = DB.add(DB.tasks, task);
  if (!added) return; // 保存失败则中止
  // 强制重新读取并验证
  state.tasks = DB.get(DB.tasks);
  const verified = state.tasks.find(t => t.id === added.id);
  if (!verified) {
    console.error('任务保存后验证失败：未在存储中找到任务', added.id);
    showToast('任务保存失败，请关闭隐私模式后重试', 'error');
    return;
  }
